...
| **Debug-Modus aktiv** | `common.debug` ist gesetzt. Kein Protokollschalter: Der javascript-Adapter unterdrückt jede schreibende Operation — `setState`, `exec`, `writeFile`, `setObject`, `deleteObject`, das Starten und Stoppen von Skripten und weitere passieren nicht, sondern werden nur als Warnung protokolliert (`was not executed, while debug mode is active`, siehe `src/lib/sandbox.ts`). Das Skript läuft und bewirkt nichts. Gilt für jede Sprache, nicht nur Blockly. |
